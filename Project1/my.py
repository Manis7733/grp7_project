...
def f5():
    ...
    try:
        # con = connect("pro.db")
        con = m.connect(host="localhost", user="root", password="Manish@7733", database="pro")
        cursor = con.cursor()
...
def u_save():
    ...
    try:
        ...
        id = uw_ent_eid.get() #Gets the text Entered by user
        if not (id.isdigit()):
...
def chart():
    ...
    try:
        con = m.connect(host="localhost", user="root", password="Manish@7733", database="pro")
        cursor = con.cursor()
        sql = "select name, sal from employee order by sal desc"
        cursor.execute(sql)
        data = cursor.fetchall()
        name = []
        salary = []
...
def get_temp():
    wa = "https://ipinfo.io"
    res = requests.get(wa)  # send a GET request to the URL
    #	print(res)
    data = res.json()  # processing the API response data to JSON
    # print(data)
    city = data["city"]
    # print("city = ", city)
    state = data["region"]
...
mw_btn_add.pack(pady=y)  #Padding from each other
...
aw_btn_back = Button(aw, text="Back", font=f, bg="khaki1", command=f2)
aw_btn_back.pack(pady=y)

aw.withdraw()  # Makes the window MINIMIZE/HIDDEN FROM USER

# VIEW EMPLOYEE BUTTON WINDOW
...
#ADDS a scrolling window for our data
vw_st_data = ScrolledText(vw, width=36, height=11, font=f)
